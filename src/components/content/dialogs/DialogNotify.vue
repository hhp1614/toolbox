<template>
  <div id="dialog-notify" class="mdui-dialog">
    ...
    <div class="mdui-dialog-content">
      <div>
        <mdui-switch label="开关" :checked="notify.enable" @change="acUpdateNotify({ enable: !notify.enable })" />
      </div>
      <div>
        <mdui-checkbox
          v-for="i in 7"
      ...
      <div class="flex">
        <div class="mdui-textfield flex-item">
          ...
          <input class="mdui-textfield-input" type="number" :value="notify.hour" @blur="updateHour" />
        </div>
        <div class="mdui-textfield flex-item">
          <label class="mdui-textfield-label">分</label>
          <input class="mdui-textfield-input" type="number" :value="notify.minute" @blur="updateMinute" />
        </div>
        <div class="mdui-textfield flex-item">
          <label class="mdui-textfield-label">秒</label>
          <input class="mdui-textfield-input" type="number" :value="notify.second" @blur="updateSecond" />
        </div>
      </div>
      <div class="mdui-textfield">
        <label class="mdui-textfield-label">提示内容</label>
        <textarea class="mdui-textfield-input" :value="notify.text" @input="updateText"></textarea>
      </div>
    </div>
  </div>
</template>
